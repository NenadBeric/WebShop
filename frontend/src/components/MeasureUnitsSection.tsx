...
export type MeasureUnitRow = { id: number; tenant_id: string; name: string; sort_order: number };

export function MeasureUnitsSection({ onChanged }: { onChanged?: () => void }) {
  const { t } = useI18n();
  ...
  async function del(id: number, nm: string) {
    ...
    setErr(null);
    try {
      await apiFetch(`/api/v1/measure-units/${id}`, { method: "DELETE" });
      await load();
      onChanged?.();
    } catch (ex) {
      setErr(ex instanceof Error ? ex.message : String(ex));
  ...
  return (
    <div className="card" style={{ marginBottom: "1rem" }}>
      <h3 style={{ marginTop: 0 }}>{t("manager.units_title")}</h3>
