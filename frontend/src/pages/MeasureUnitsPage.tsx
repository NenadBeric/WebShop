...
export function MeasureUnitsPage() {
  const { t } = useI18n();
  const [rows, setRows] = useState<MeasureUnitRow[]>([]);
  ...
  async function onAdd(e: FormEvent) {
    ...
    setErr(null);
    await apiFetch("/api/v1/measure-units", {
      method: "POST",
  ...
  function startEdit(r: MeasureUnitRow) {
    setEditingId(r.id);
    setEditName(r.name);
  ...
  async function saveEdit() {
    ...
    setErr(null);
    await apiFetch(`/api/v1/measure-units/${editingId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function del(r: MeasureUnitRow) {
    if (!confirm(`${t("manager.delete")} „${r.name}”?`)) return;
    setErr(null);
    await apiFetch(`/api/v1/measure-units/${r.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <h1 style={{ marginTop: 0 }}>{t("ref.units_title")}</h1>
      ...
      <form className="card" onSubmit={onAdd} style={{ marginBottom: "1rem" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", alignItems: "flex-end" }}>
          <div className="field" style={{ marginBottom: 0, minWidth: "12rem" }}>
            <label>{t("manager.unit_name")}</label>
            ...
            {rows.map((r) =>
              editingId === r.id ? (
                <tr key={r.id}>
                  ...
                  <td data-label={t("manager.unit_name")}>
                ...
                <tr key={r.id}>
                  <td data-label={t("common.id")}>{r.id}</td>
                  <td data-label={t("manager.unit_name")}>{r.name}</td>
